...
2. **Extract and enumerate functional requirements.** List every discrete capability the system must provide. Use the user's language but restructure it into a numbered list of concrete feature statements. Each statement should follow the pattern: _"The system must allow [actor] to [action] so that [outcome]."_ If the user has not provided enough detail, ask targeted clarifying questions — never assume critical functionality.
